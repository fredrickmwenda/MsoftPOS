...
@section('content')
    <section>
        <div class="container-fluid">
            <h4 class="mt-3 mb-4">{{ __("file.HRM - Generate Payroll") }}</h4>
            ...
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-body">
                    <div class="row align-items-end g-3">
                        <div class="col-md-4">
                            <h5 class="fw-bold mb-2">
                                {{ __("file.Payroll for") }} <strong>{{ date('F Y', strtotime($month)) }}</strong>
                            </h5>
                            <small class="text-muted d-block">
                                <b>{{ __("file.Location") }}</b> :
                                {{ $warehouse_id ? $warehouse->name : __("file.All Warehouse") }}
                        ...
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">
                                {{ __("file.Payroll Group Name") }} <span class="text-danger">*</span>
                            </label>
                            <input type="text" class="form-control" name="payroll_group_name" required
                                value="{{ __("file.Payroll for") }} {{ date('F Y', strtotime($month)) }}">
                        </div>
                    ...
                    <div class="col-md-4">
                        ...
                        {{-- <div class="col-md-4">
                            <label class="form-label fw-semibold">
                                {{ __("file.status") }} <span class="text-danger">*</span>
                            ...
                            <select name="payroll_group_status" class="form-select select2 payroll-group-status">
                                <option value="">{{ __("file.Change Status") }}</option>
                                <option value="draft">{{ __("file.Draft") }}</option>
                                <option value="final">{{ __("file.final") }}</option>
                            </select>
                            <small class="text-muted d-block mt-1">
                                {{ __("file.Payroll cannot be deleted if status is final") }}.
                            </small>
                        </div> --}}
                    </div>
                </div>
                                ...
                                <div class="card mb-3 shadow-sm border-0">
                                    <div class="card-header bg-light">
                                        ...
                                        <small class="text-muted ms-2">
                                            {{ __("file.employee_id") }}: {{ $employee->id }}
                                        </small>
                                    </div>
                                        ...
                                        <table class="table table-bordered align-middle mb-0">
                                            <tr>
                                                <td style="width:30%">
                                                    ...
                                                    <b>{{ __("file.Leaves") }} :</b> {{ $employee->total_leaves ?? 0 }} {{ __("file.Days") }} <br><br>
                                                    <b>{{ __("file.Work Duration") }} :</b> {{ $employee->total_work_hours ?? '0.00' }} {{ __("file.Hours") }} <br><br>
                                                    <b>{{ __("file.Attendance") }} :</b> {{ $employee->attendance_days ?? 0 }} {{ __("file.Days") }}
                                                </td>
                                                ...
                                                <td>
                                                    <div class="row g-2">
                                                        ...
                                                        {{-- Commission --}}
                                                        <div class="col-md-2">
                                                            <label>{{ __("file.Commission") }}</label>
                                                            <input type="number" step="any"
                                                        ...
                                                        {{-- Total --}}
                                                        <div class="col-md-3">
                                                            <label>{{ __("file.Total Payable") }}</label>
                                                            <input type="number" step="any"
                                                        ...
                                                        {{-- Status --}}
                                                        <div class="col-md-3">
                                                            <label>{{ __("file.Select Status") }}</label>
            ...
            <div class="text-end mt-3">
                <button type="submit" class="btn btn-primary px-4">
                    <i class="dripicons-checkmark me-1"></i> {{ __("file.Submit All Payrolls") }}
                </button>
            </div>
