            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        ...
                        <div>
                            <a href="{{ route('hire_purchase.pending') }}" class="btn btn-sm btn-primary">
                                <i class="dripicons-list"></i> Pending
                            </a>
                    ...
                    <div class="card-body">
                        @if($installments->count() > 0)
                            <div class="alert alert-danger">
                                ...
                                <br>
                                You have {{ $installments->count() }} overdue installment(s) requiring immediate attention.
                                <br>
                                Total overdue amount: {{ number_format($installments->sum('amount'), 2) }}
                            </div>
                        @endif
                                ...
                                <tbody>
                                    @forelse($installments as $installment)
                                        <tr class="table-danger">
                                            <td>
                                                <strong>{{ $installment->sale->reference_no }}</strong>
                                            </td>
                                            <td>{{ $installment->sale->customer->name ?? 'N/A' }}</td>
                                            <td>{{ $installment->sale->customer->phone_number ?? 'N/A' }}</td>
                                            ...
                                            <td>
                                                <strong>{{ number_format($installment->remaining_amount, 2) }}</strong>
                                            </td>
                                            <td>
                                        ...
                                        <tr>
                                            <td colspan="10" class="text-center text-muted">
                                                <p class="mt-3">
                                                    ...
                                                    {{trans('file.No overdue installments - Great job!')}}
                                                </p>
                                            </td>
                                        </tr>
                                    @endforelse
