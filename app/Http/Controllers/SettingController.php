    ...
    public function generalSettingStore(Request $request)
    {
       ...
       // dd($data);
        ...
        $general_setting->developed_by = $data['developed_by'];
         $general_setting->whatsapp_number = $request->whatsapp_number;
        ...
        if ($favicon) {
            $this->fileDelete('logo/', $general_setting->site_favicon);

            $ext = pathinfo($favicon->getClientOriginalName(), PATHINFO_EXTENSION);
            $logoName = date("Ymdhis") . '2.' . $ext;
            $favicon->move('public/logo', $logoName);
