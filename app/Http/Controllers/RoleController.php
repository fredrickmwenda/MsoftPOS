...
class RoleController extends Controller
{
    ...
    public function permission($id)
    {
        if ($this->userIsAdmin()) {
            $lims_role_data = Roles::find($id);
            // get permission names directly from our Roles model
            $all_permission = $lims_role_data->permissions->pluck('name')->toArray();

            // ensure some new permissions exist and are assigned
            $newPermissions = ['approvals-index', 'sale-percentage-filter'];
            foreach ($newPermissions as $permName) {
                if (!in_array($permName, $all_permission)) {
                    $perm = Permission::firstOrCreate(['name' => $permName]);
                    $lims_role_data->givePermissionTo($perm);
                    $all_permission[] = $permName;
                }
            }
    ...
    //     $role = Roles::firstOrCreate(['id' => $request['role_id']]);
    ...
    public function setPermission(Request $request)
    {
        ...
        // Use our own Roles model – firstOrCreate by ID
        $role = Roles::firstOrCreate(['id' => $request['role_id']]);

        // Helper to toggle a permission based on checkbox presence
        $togglePermission = function ($permissionName) use ($role, $request) {
        ...
        // Toggle all permissions – list taken from your original code
        $permissionsList = [
