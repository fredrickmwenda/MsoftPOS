...
class TransferController extends Controller
{
    ...
    public function getProduct($id)
    {
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse)
        {
            ...
            $product_price[] = $product_warehouse->price ?? 'NA';
            $lims_product_data = Product::select('name', 'code', 'price', 'shelf')->find($product_warehouse->product_id);
            $product_code[] =  $lims_product_data->code;
            $product_name[] = $lims_product_data->name;            
            $product_shelf[] = $lims_product_data->shelf ?? 'NA';;
            // if($lims_product_data->shelf)
            //     $product_name[count($product_name) - 1] .= ' [' . $lims_product_data->shelf . ']';
            // else
            //     $product_name[count($product_name) - 1] .= ' [N/A]';
        ...
        foreach ($lims_product_with_variant_warehouse_data as $product_warehouse)
        {
            $product_qty[] = $product_warehouse->qty;
            $product_price[] = $product_warehouse->price ?? 'NA';
            $lims_product_data = Product::select('name', 'code', 'shelf')->find($product_warehouse->product_id);
            $lims_product_variant_data = ProductVariant::select('item_code')->FindExactProduct($product_warehouse->product_id, $product_warehouse->variant_id)->first();
            $product_code[] =  $lims_product_variant_data->item_code;
            $product_name[] = $lims_product_data->name;
            $product_shelf[] = $lims_product_data->shelf ?? 'NA';;
        }
        $product_data = [$product_code, $product_name, $product_qty, $product_price, $product_shelf];
        return $product_data;
    }
