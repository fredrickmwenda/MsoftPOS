...
class WhatsappController extends Controller
{
    ...
    public function updateSettings(Request $request)
    {
        ...
        return redirect()->back()->with('message', __('file.Data updated successfully'));
    }
    ...
    public function templates()
    {
        ...
        if (!$settings || empty($settings->business_account_id) || empty($settings->permanent_access_token)) {
            $templates = [];
            session()->now('not_permitted', __('file.whatsapp_credentials_missing'));
    ...
    public function sendMessage(Request $request)
    {
        ...
        if (!$settings || empty($settings->phone_number_id) || empty($settings->permanent_access_token)) {
            return back()->with('not_permitted', __('file.whatsapp_credentials_missing'));
        }
        ...
        if ($request->has('_from_form')) {
            ...
            if ($result['success'] ?? false) {
                ...
                return back()->with('not_permitted', $result['message'] ?? __('file.fail_sent_message'));
            }
        }
        return $result;
    }
